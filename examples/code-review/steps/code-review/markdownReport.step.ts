...
export const handler: StepHandler<typeof config> = async (input: MarkdownReportInput, { emit, logger, state, traceId }) => {
  try {
    ...
    // Check for fallback reports that have already been generated
    if ('fallbackReport' in input && (input as any).fallbackReport === true) {
      logger.info('Fallback report already generated by controller, skipping markdown generation');
      
      // Just emit the completion event without doing anything
      await emit({
        topic: 'code-review.report.generated',
        data: {
          filepath: outputPath || path.join(process.cwd(), 'Review.md'),
          content: 'Fallback report already generated'
        }
      });
